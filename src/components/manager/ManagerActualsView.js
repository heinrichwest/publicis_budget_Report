...
const ManagerActualsView = () => {
  ...
  const [editingCell, setEditingCell] = useState(null); // { actualId, month, field }
  ...
  const startEditing = (actualId, month, field, currentValue) => {
    setEditingCell({ actualId, month, field });
  ...
  const saveEdit = async () => {
    ...
    const { actualId, month, field } = editingCell;
    const newValue = parseFloat(editValue) || 0;

    // Update local state first
    setActuals(prevActuals =>
      prevActuals.map(actual => {
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      const updatePromises = actuals.map(actual => {
            ...
            {filteredActuals.map(actual => {
              ...
              return (
                <tr key={actual.id} style={styles.tr}>
                  ...
                  {MONTHS.map((month, monthIndex) => {
                    const monthData = actual.monthlyActuals?.[month] || { rateCard: 0, discount: 0, addedValue: 0 };
                    const rawRateCard = parseFloat(monthData.rateCard) || 0;
                    const rawDiscount = parseFloat(monthData.discount) || 0;
                    const rawAddedValue = parseFloat(monthData.addedValue) || 0;
                    const rateCard = convertToZAR(rawRateCard, actual.market);
                    const discount = convertToZAR(rawDiscount, actual.market);
                    const addedValue = convertToZAR(rawAddedValue, actual.market);
                    const nett = rateCard - discount;
                    const discPerc = rateCard > 0 ? (discount / rateCard) * 100 : 0;
                    const avPerc = rateCard > 0 ? (addedValue / rateCard) * 100 : 0;

                    const isEditingRateCard = editingCell?.actualId === actual.id && editingCell?.month === month && editingCell?.field === 'rateCard';
                    const isEditingDiscount = editingCell?.actualId === actual.id && editingCell?.month === month && editingCell?.field === 'discount';
                    const isEditingAddedValue = editingCell?.actualId === actual.id && editingCell?.month === month && editingCell?.field === 'addedValue';

                    return (
                      <React.Fragment key={month}>
                        {/* Rate Card - Editable */}
                        <td style={styles.tdEditable} onClick={() => !isEditingRateCard && startEditing(actual.id, month, 'rateCard', rawRateCard)}>
                          {isEditingRateCard ? (
                            <input
                              type="number"
                              value={editValue}
                        ...
                        {/* Discount - Editable */}
                        <td style={styles.tdEditable} onClick={() => !isEditingDiscount && startEditing(actual.id, month, 'discount', rawDiscount)}>
                          {isEditingDiscount ? (
                            <input
                              type="number"
                              value={editValue}
                        ...
                        {/* Disc % - Calculated, not editable */}
                        <td style={styles.td}>{discPerc.toFixed(2)}%</td>
                        {/* Nett - Calculated, not editable */}
                        <td style={styles.td}>{formatNumber(nett)}</td>
                        {/* Added Value - Editable */}
                        <td style={styles.tdEditable} onClick={() => !isEditingAddedValue && startEditing(actual.id, month, 'addedValue', rawAddedValue)}>
                          {isEditingAddedValue ? (
                            <input
                              type="number"
                              value={editValue}
